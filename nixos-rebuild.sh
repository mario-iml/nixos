...
# Rebuild, output simplified errors, log trackebacks - uses shell alias set in device specific nix configs
rebuild-command &>nixos-switch.log || (cat nixos-switch.log | grep --color error && exit 1)

# Get current generation metadata
current=$(nixos-rebuild list-generations --flake ~/nixos/ | grep current)
